...
class Process extends StatefulWidget {
  @override
  _Process createState() => _Process();
}

class _Process extends State<Process> {
  ...
  Future<void> _generateAndPrintPDF() async {
    ...
    Uint8List imageData = await getImageDataFromAssets('assets/images/background.png');
    PdfBitmap image = PdfBitmap(imageData);

    // Add a page to the document.
    final PdfPage firstPage = document.pages.add();

    // Add the image to the top of the first page and align to center.
    final double centerX = (firstPage.getClientSize().width - 80) / 2;
    firstPage.graphics.drawImage(image, Rect.fromLTWH(centerX, 0, 120, 120));

    // Add university information and student details to the document.
    final String rollNo = batches.isNotEmpty ? batches[0]['student_id'].toString() : '';
    final String name = batches.isNotEmpty ? batches[0]['student_name'].toString() : '';
    final String email = batches.isNotEmpty ? batches[0]['student_email'].toString() : '';
    final String contact = batches.isNotEmpty ? batches[0]['student_contact'].toString() : '';
    final String batch = batches.isNotEmpty ? batches[0]['batch_id'].toString() : '';
    final String department = batches.isNotEmpty ? batches[0]['depart_id'].toString() : '';
    final String address = batches.isNotEmpty ? batches[0]['student_address'].toString() : '';

    // University information and student details container.
    final String studentInfoText = 'University Information:\n'
        'Roll No: $rollNo\n'
        'Name: $name\n'
        'Email: $email\n'
        'Contact: $contact\n'
    ...
    final double textHeight = PdfStandardFont(PdfFontFamily.timesRoman, 12).height * 7;
    firstPage.graphics.drawString(studentInfoText, PdfStandardFont(PdfFontFamily.timesRoman, 12),
        bounds: Rect.fromLTWH(50, 100, firstPage.getClientSize().width - 100, textHeight));

    // Add a paragraph for the header "Selected Courses".
    firstPage.graphics.drawString('Selected Courses:',
        PdfStandardFont(PdfFontFamily.timesRoman, 18),
        bounds: Rect.fromLTWH(50, 100 + textHeight + 10, 0, 0));

    // Create a grid for the course information.
    ...
    header.cells[0].value = 'Course ID';
    header.cells[1].value = 'Course Name';
    header.cells[2].value = 'Section';
    header.cells[3].value = 'Course Type';
    header.cells[4].value = 'Course Pre-reg';
    header.cells[5].value = 'Department';
    ...
    // Add rows to the grid with course information.
    for (final cr in course) {
      PdfGridRow row = grid.rows.add();
      row.cells[0].value = cr['CourseID'].toString();
      row.cells[1].value = cr['Course_Name'].toString();
      row.cells[2].value = cr['section'].toString();
      row.cells[3].value = cr['course_type'].toString();
      row.cells[4].value = cr['Course_Pre_reg'].toString();
      row.cells[5].value = cr['depart_id'].toString();
    }
    ...
    // Draw the grid on the first page.
    final double gridHeight = firstPage.getClientSize().height - (100 + textHeight + 50);
    grid.draw(
        page: firstPage,
        bounds: Rect.fromLTWH(50, 100 + textHeight + 30, firstPage.getClientSize().width - 100, gridHeight)
    );

    int start = 0;

    for (final semesterDetails in semesterDetailsList) {
      final PdfPage semesterPage = document.pages.add();

      // Add semester information to the page.
      semesterPage.graphics.drawString(semesterDetails.semester.toString(),
          PdfStandardFont(PdfFontFamily.timesRoman, 18),
          bounds: Rect.fromLTWH(50, 50, 0, 0));
      ...
      // Create a grid for the semester courses.
      PdfGrid semesterGrid = PdfGrid();
      semesterGrid.columns.add(count: 5);

      // Add headers to the semester grid.
      semesterGrid.headers.add(1);
      PdfGridRow semesterHeader = semesterGrid.headers[0];
      semesterHeader.cells[0].value = 'Course ID';
      semesterHeader.cells[1].value = 'Course Name';
      semesterHeader.cells[2].value = 'Grade';
      semesterHeader.cells[3].value = 'Section';
      semesterHeader.cells[4].value = 'Credit Hours';

      // Add rows to the semester grid with course information.
      for (final course in semesterCourses) {
        PdfGridRow row = semesterGrid.rows.add();
        row.cells[0].value = course['courseId'] ?? '';
        row.cells[1].value = course['Course_Name'] ?? '';
        row.cells[2].value = course['grade'] ?? '';
        row.cells[3].value = course['section'] ?? '';
        row.cells[4].value = course['creditHours'] ?? 'null';
      }

      // Customize semester grid style.
      semesterGrid.style = PdfGridStyle(
        cellPadding: PdfPaddings(left: 10, right: 3, top: 4, bottom: 5),
        backgroundBrush: PdfBrushes.lightGray,
        textBrush: PdfBrushes.black,
      ...
      // Draw the semester grid on the page.
      semesterGrid.draw(
          page: semesterPage,
          bounds: Rect.fromLTWH(50, 100, semesterPage.getClientSize().width - 100, semesterPage.getClientSize().height - 150)
      );
    }
    ...
    html.AnchorElement(
      ...
      ..setAttribute("download", '${rollNo}' + "_Registration_Application.pdf")
      ..click();
  ...
  // pw.Table _buildTranscriptTable() {
  //   getTranscript();
  //   return pw.Table.fromTextArray(
  //     data: <List<String>>[
  //       <String>['Course ID', 'Course Name', 'Grade', 'Section', 'Credit Hours'],
  //       ...TranscriptInfo.map((transcript) => [
  //         transcript['courseId'].toString(),
  //         transcript['Course_Name'].toString(),
  //         transcript['grade'].toString(),
  //         transcript['section'].toString(),
  //         transcript['creditHours'].toString(),
  //       ]),
  //     ],
  //   );
  // }
  ...
  bool isApplicationProcessed() {
    ...
    return applications.isNotEmpty && (applications[0]['isProcessed'] );
  }
